...
/*
	...
	a loading screen, or B) stream in the entire audio into memory on another thread. tsReadMemOGG is
	...
	while (*ptr)
	{
		...
		if (playing->sample_index == loaded->sample_count)
		{
			...
			*ptr = (*ptr)->next;
			...
			TS_ASSERT(playing->loaded_sound->playing_count >= 0);
